...
Additionally, as the challenge requested for a mode specifically for the export function, as such, it was assumed that the toolbar only required the export tool as the otherwise a separate mode would not be needed and it could simply be added to the toolbar of other modes. In the future, I would want to implement the export button in this manner such that the user can manipulate the image and then export it as opposed to switching modes to do so.
